...
if __name__ == "__main__":
    ...
    with Model("lo1") as model:
        # Create variable 'x' of length 4
        x = model.variable("x", 4, Domain.greaterThan(0.0))

        # Create constraints
        # Constraint: x[1] <= 10.0 (upper bound on the second variable)
        model.constraint(x.index(1), Domain.lessThan(10.0))
        # Constraint: A[0] * x == 30.0 (equality constraint)
        model.constraint("c1", Expr.dot(A[0], x), Domain.equalsTo(30.0))
        # Constraint: A[1] * x >= 15.0 (lower bound constraint)
        model.constraint("c2", Expr.dot(A[1], x), Domain.greaterThan(15.0))
        # Constraint: A[2] * x <= 25.0 (upper bound constraint)
        model.constraint("c3", Expr.dot(A[2], x), Domain.lessThan(25.0))

        # Set the objective function to maximize (c^T * x)
        model.objective("obj", ObjectiveSense.Maximize, Expr.dot(c, x))
        ...
        print("\n".join([f"x[{i}] = {sol[i]}" for i in range(4)]))
