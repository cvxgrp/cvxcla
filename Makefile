...
# ---------------------------------------------------------------------------
# Repo-owned additions below. Everything above is `uvx rhiza-task shim`.
# ---------------------------------------------------------------------------

# `book/` exists in this repo (book/marimo/notebooks), so it shadows the `book`
# task through the catch-all above -- the shim header's "none do" is not true
# here. The failure is mtime-driven and therefore intermittent: `%: $(UVX)`
# gives the target a prerequisite, so make compares `book/` against the uvx
# binary and, whenever the directory is the newer of the two, declares
# "`book' is up to date." and builds nothing. Editing a notebook is enough to
# put it in that state, while a freshly installed ./bin/uvx hides it again.
# Verified both ways on an isolated fixture before adding this rule.
#
# An explicit rule beats a pattern rule, which restores delegation
# unconditionally. Only `book` collides today; docs/, src/, tests/, bin/ and
# experiments/ match no task name.
.PHONY: book
book: $(UVX)
	@$(UVX) $(RHIZA_TASK) book
