...
class CLA(InequalityConstrained):
    ...
    @classmethod
    def problem(
        cls, mean: NDArray[np.float64], covariance: NDArray[np.float64] | QuadraticForm
    ) -> "ProblemBuilder[CLA]":
        """Start a fluent :class:`cvxcla._builders.ProblemBuilder` for this problem.
        ...
        what lets the builder live in a leaf module that never imports this one.
        ...
        Returns:
            A ``ProblemBuilder`` ready to accept constraints, whose ``.trace()``
            returns a solved ``CLA``.
        """
        return ProblemBuilder(mean, covariance, solver=cls)
