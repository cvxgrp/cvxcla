...
class ProblemBuilder:
    ...
    A thin, chainable convenience layer over the explicit :class:`CLA`
    constructor. It exists purely for readability: portfolio practitioners expect
    to say "long-only, fully invested" rather than to remember that the budget is
    encoded as ``a=np.ones((1, n)), b=np.ones(1)``. Every method maps one-to-one
    onto a constructor argument, so the builder adds no modelling power and imposes
    no expression algebra: it accepts the same polyhedral pieces the CLA already
    ...
    Construct one via :meth:`CLA.problem`, chain the constraint methods (each
    returns ``self``), and finish with :meth:`trace`, which builds the ``CLA`` and
    runs the full parametric trace, returning the solved object whose ``frontier``
    and ``turning_points`` describe the entire efficient frontier (not a single
    optimum, which is the distinction from a one-shot convex solver).
    ...
    def trace(self) -> CLA:
        ...
        Returns:
            The solved :class:`CLA`, whose ``frontier`` and ``turning_points``
            describe the entire efficient frontier.
