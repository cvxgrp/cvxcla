...
out-of-bounds drift, and sub-tolerance round-off at degenerate vertices is
projected back onto the feasible set so near-rank-deficient problems still
complete with optimal turning points (\S\ref{sec:empirical}).
\end{itemize}
...
\S\ref{sec:exactness}) in objective to $\sim\!10^{-9}$
...
The repair is deliberately bounded. A \emph{small} box violation is round-off and
is projected away; a \emph{gross} one (order $10^{-1}$) signals a genuinely
rank-deficient free block, whose solve is unreliable once the free set grows past
the covariance rank. Projecting such a point would silently return a suboptimal
frontier, so a guard (at $10^{-2}$, an order of magnitude above the round-off
scale and well below the broken-solve scale) refuses it and raises an actionable
diagnosis instead. This is the precise guarantee: while the free-asset block
stays full rank the trace completes and every turning point is optimal; when it
does not, the algorithm declines rather than mislead. A well-conditioned,
full-rank estimate (ample history, or a structured \texttt{FactorCovariance},
which is positive definite by construction) stays in the first regime. The
...
falls as $T$ shrinks (leftward). Each marker is the worst box violation of a
\emph{candidate} turning point, before projection. Blue circles complete the full
trace, and every completed frontier is optimal, matching a reference QP in
objective to $\sim\!10^{-9}$. Red crosses abort. Completions sit below the
projection guard (dashed, $10^{-2}$): their violations are round-off in the
covariance's near-flat directions, repaired by projecting onto the feasible box.
Aborts sit above it: the free block has grown past the covariance rank and its
solve is unreliable, so the algorithm declines rather than return a suboptimal
frontier. Completion is not perfectly monotone in $T$ because degeneracy is
sporadic.}
