...
($\approx n^{3.4}$): at $n=640$ it takes about $264$~seconds, so \texttt{cvxcla}'s
dense backend is roughly $340\times$ faster and the factor backend roughly
$2900\times$ faster. This gap is not a \texttt{numpy}-versus-Python artefact ---
...
$8\times$ faster than the dense backend. The incremental-inverse baseline and the
dense backend lie close together, far below PyPortfolioOpt.}
\label{fig:scaling}
\end{figure}
...
The answer is yes: the incremental-inverse baseline ($\approx n^{2.0}$, $326$~ms at
$n=640$) is about $2.4\times$ \emph{faster} than the dense backend
($\approx n^{2.2}$, $775$~ms), exactly as the $O(n_\F^2)$-versus-$O(n_\F^3)$ count
...
backend more than repays it: at $n=640$ it is $\approx 8.4\times$ faster than the
dense backend and $\approx 3.5\times$ faster than the incremental-inverse baseline,
...
\paragraph{Remark on a real degeneracy, and what it is not.}
...
partway through the trace rather than completing it. It is tempting to read this
as a conditioning or rank failure, but a closer look shows it is neither.

Three observations locate the true cause (Figure~\ref{fig:degeneracy}). First, at
the point where the trace stops, the free-asset block $\Sig_{\F\F}$ is
\emph{well} conditioned: its condition number is of order $10^2$ to $10^3$,
nowhere near singular, and the reduced KKT solve is accurate. Second, linear
shrinkage toward a scaled identity, which restores a positive-definite,
well-conditioned ($\kappa \approx 90$) covariance, does \emph{not} rescue the
trace; it still aborts. Third, completion is not monotone in the window length or
the rank: a $100$-day window (sample rank $99 \ll N$) completes, while a
$550$-day, \emph{full-rank} window aborts. What every abort has in common is
small and specific: a single free weight, sitting essentially on one of its
bounds at a degenerate vertex, is placed just outside its box, by an amount of
order $10^{-5}$ (between roughly one and ten times the feasibility tolerance
$\tau$), through accumulated floating-point round-off over the hundreds of
turning points of a large trace. The cause is coincident-event handling and
round-off, not conditioning, so covariance shrinkage alone is not the remedy.

The abort is the conservative choice rather than a bug. Forcing the trace past
the violation, by projecting the offending weight back onto its bound and
continuing, does complete, but the resulting walk has a non-monotone $\lambda$
and its frontier departs from a reference QP solve by up to $\sim\!10^{-3}$, three
to five orders of magnitude worse than the $\sim\!10^{-8}$ agreement of the
well-posed case (\S\ref{sec:exactness}). The algorithm therefore stops rather
than emit a turning point it cannot certify feasible. The guarantee we can state
is correspondingly precise: when the events are well separated relative to $\tau$,
so that no turning-point weight lands within round-off of a bound it is not
pinned to, the trace is exact (\S\ref{sec:exactness}); a well-conditioned,
well-separated estimate (ample history, or a structured \texttt{FactorCovariance})
meets this condition, whereas a tie-heavy or near-rank-deficient sample covariance
may not. The Bland-style rule of \S\ref{sec:bland} and the
$\sqrt{\varepsilon_{\text{mach}}}$ slope floor of \S\ref{sec:events} harden the
common cases; hardening the remaining tie-heavy regime (for example with a
lexicographic perturbation of coincident events) is left to future work. The
experiment is reproducible from \texttt{experiments/degeneracy\_boundary.py}.

\begin{figure}[h]
\centering
\includegraphics[width=0.72\textwidth]{degeneracy.pdf}
\caption{Where the CLA trace aborts on the S\&P 500 universe as the
estimation-window length $W$ varies (log--log). Markers show the worst
box-bound violation of the candidate turning point at which the trace stops:
blue circles complete the full trace (violation at floating-point floor), red
crosses abort. Every abort sits just above the feasibility tolerance $\tau$
(dashed line), at a violation of order $10^{-5}$, while the free-block condition
number (green, right axis) stays of order $10^2$ to $10^3$. Completion is not
monotone in $W$: short rank-deficient windows can complete and long full-rank
windows can abort, confirming that the failure is accumulated round-off at
degenerate vertices, not a singular free block.}
