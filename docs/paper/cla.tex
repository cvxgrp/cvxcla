...
Riskfolio-Lib \citep{riskfolio} and
scikit-portfolio,\footnote{\url{https://github.com/scikit-portfolio/scikit-portfolio}}
and in R \texttt{fPortfolio} \citep{fportfolio} and the
...
away as $n$ grows: from rough parity at $n=40$ to a $\approx 7.7\times$ speedup
at $n=640$ (Table~\ref{tab:scaling}). This $\approx 7.7\times$ is the genuine
...
dense backend is roughly $322\times$ faster and the factor backend roughly
$2460\times$ faster. This gap is not a \texttt{numpy}-versus-Python artefact (
...
$n = 640$ the factor backend is roughly $2460\times$ faster than PyPortfolioOpt and
$7.7\times$ faster than the dense backend. The incremental-inverse baseline and the
...
The $\approx 322\times$ gap between \texttt{cvxcla}'s dense backend and
...
The answer is yes: the incremental-inverse baseline ($\approx n^{2.0}$, $329$~ms at
$n=640$) is about $2.4\times$ \emph{faster} than the dense backend
($\approx n^{2.2}$, $792$~ms), exactly as the $O(n_\F^2)$-versus-$O(n_\F^3)$ count
...
backend more than repays it: at $n=640$ it is $\approx 7.7\times$ faster than the
dense backend and $\approx 3.2\times$ faster than the incremental-inverse baseline,
...
the gap widens with $n$: from $\approx 1.7\times$ at $n=20$ to $\approx 59\times$ at
$n=320$, where one \texttt{cvxcla} trace takes $\approx 0.12$~s and the $320$
Clarabel solves take $\approx 7.4$~s ($\approx 23$~ms each). The sweep grows like
...
$\approx 59\times$ slower than the trace at $n=320$, and yields only an $n$-point
