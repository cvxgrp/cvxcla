...
% Code input (Python) and console output styles for the usage section.
\lstdefinestyle{pyin}{%
  language=Python,
...
\newcommand{\srcref}{v1.8.1}
\newcommand{\srcfile}[2]{\href{\repourl/blob/\srcref/#1}{\texttt{#2}}}

\title{\textbf{The Critical Line Algorithm}\\[0.3em]
\large An operator-based implementation for the constrained mean--variance efficient
frontier in \texttt{cvxcla}}
...
low-rank. Second, \emph{generality and reuse}: the turning-point loop is written for
box bounds together with arbitrary linear equality ($Aw=b$) and inequality
($Gw\le h$) constraints, and, because it reaches the covariance only as an operator,
the same engine drives a reusable path tracer (the package's \texttt{Lasso} class is one
client). The cross-field correspondence this reflects, and its history, are the subject
of a companion paper \citep{schmelzer2026perspective}; here we treat only the
implementation. Third, an \emph{open, tested, and benchmarked} implementation, with event logic
...
Algorithm (CLA) of \citet{markowitz1956, markowitz1959} computes this frontier
...
Classical CLA implementations assume a dense covariance. In practice it is almost never
dense: modern multi-factor risk models, generalising the single-index model
\citep{sharpe1963}, write the covariance as a diagonal matrix plus a low-rank term, and
it is precisely this diagonal-plus-low-rank structure that the operator abstraction of
this paper exploits.
...
\item \textbf{Generality and a reusable tracer.} The turning-point loop is written for
box bounds together with arbitrary linear equality ($A w = b$) and inequality
($G w \le h$) constraints, an active inequality row entering the same reduced solve as
an extra equality row at no cost to the box-only problem (\S\ref{sec:lars}). Because the
loop reaches the covariance only through the operator interface, it factors into a
problem-agnostic path tracer of which the package's \texttt{Lasso} class is a second
client (\S\ref{sec:lars}). The broader cross-field correspondence this reflects is the
subject of a companion paper \citep{schmelzer2026perspective}, not of this one.
...
(\S\ref{sec:empirical}), while two cases are declined with an actionable diagnosis
rather than mis-solved: a numerically rank-deficient free block
(\S\ref{sec:empirical}), and a degenerate maximum-return vertex under a general
equality system, where the free set is too small to span the constraints
(\S\ref{sec:bailey}). The lowest-index tie-break (\S\ref{sec:bland}) keeps the
...
turning points are its breakpoints.
...
a vertex; the free set is read off as the assets strictly inside their bounds. When the
maximum-return face is degenerate (several vertices attain the same $\mu\trans w$) the
simplex returns one of them, fixing only the $\lambda=\infty$ starting vertex: for a
positive-definite $\Sig$ the optimiser of \eqref{eq:qp} is unique at every $\lambda>0$,
so the traced frontier below the endpoint does not depend on which maximum-return vertex
is chosen. Only the
...
since the enormous ratios they would produce merely amplify rounding error. The
exact level is not delicate: genuine slopes sit orders of magnitude above
$\sqrt{\varepsilon_{\text{mach}}}$ and round-off noise orders below, so no slope lands
near the threshold and the trace is insensitive to its precise value, exactly as the
wide regime gap leaves the conditioning guard of \S\ref{sec:empirical} uncritical.
...
\S\ref{sec:bland}; the hard iteration cap of $100(n+p+1)$ is a \emph{numerical} safety net
...
the three operations with plain \texttt{numpy}/\texttt{scipy} calls: \texttt{matvec}
is $\Sig x$, \texttt{cross} is the off-diagonal block product, and \texttt{solve\_free}
solves the symmetric positive-definite system $\Sig_{\F\F}x=b$ on the principal
submatrix by Cholesky factorisation (\texttt{scipy.linalg.cho\_factor}/\texttt{cho\_solve}),
falling back to a general LU solve (\texttt{np.linalg.solve}) on the rare block that is
not numerically positive-definite. This reproduces exactly the behaviour of the
classical CLA on a raw matrix and is the reference backend.
...
(\srcfile{src/cvxcla/types.py}{types.py}). Each \texttt{FrontierPoint} carries a weight vector and
...
computed pointwise. The maximum-Sharpe portfolio is computed in closed form,
exploiting the same piecewise-linear weight structure. On an affine weight segment
$w(t) = (1-t)\,w_0 + t\,w_1$, $t\in[0,1]$, the expected return is affine and the
variance quadratic in $t$, so the Sharpe ratio
$S(t) = (a_0 + a_1 t)/\sqrt{c_0 + c_1 t + c_2 t^2}$ is an affine function over the
square root of a quadratic (here $a_0 = \mu\trans w_0$, $a_1 = \mu\trans(w_1-w_0)$,
and $c_0,c_1,c_2$ are the coefficients of $w(t)\trans\Sig\,w(t)$). Its derivative has
a \emph{linear} numerator, the quadratic terms cancelling, so each segment has a
single stationary point $t^\star = (a_0 c_1 - 2 a_1 c_0)/(a_1 c_1 - 2 a_0 c_2)$; the
maximiser is the best of $t^\star$ (when it lies in $[0,1]$) and the two segment
endpoints, taken over the two segments bracketing the discrete Sharpe maximum. Two
segments suffice because the Sharpe ratio is unimodal along the frontier: the
efficient frontier is concave and increasing in the $(\sigma,\,\mu\trans w)$ plane,
so the ray slope $\mu\trans w/\sigma$ rises to the tangency portfolio and falls
thereafter; its values at the turning points are therefore unimodal, and the turning
point of largest discrete Sharpe ratio brackets the continuous maximum. The
...
row activating or releasing. On non-degenerate problems a coordinate re-enters the
free set only finitely often, so the count is empirically $O(n)$, mirroring the
typical $O(n)$ breakpoint count of the LARS path \citep{efron2004}; the worst case is
combinatorial (as for parametric active-set and mpQP methods generally
\citep{bemporad2002}) but is not observed on
the problems of \S\ref{sec:experiment}. The implementation caps iterations at
$100(n+p+1)$ (one factor over the $n$ box bounds and $p$ inequality rows whose
activity each turning point fixes) as a numerical safety net.
...
complete with optimal turning points (\S\ref{sec:empirical}). The tie-break is robust
...
The package is installed with \texttt{pip install cvxcla} and the whole workflow
goes through a single class, \texttt{CLA}. The user supplies the mean $\mu$, the
covariance $\Sig$ (a dense array or any backend of \S\ref{sec:operators}), the box
bounds $\lb,\ub$, and the constraints: the equality system $A,b$ (always at least
the budget), and optionally the inequality system $G,h$. Constructing the object
traces the whole frontier; the turning points and the derived \texttt{Frontier}
(\S\ref{sec:frontier}) are then available as attributes. Listing~\ref{lst:basic}
traces a small long-only, fully-invested problem and reads off the maximum-Sharpe
portfolio.

\begin{lstlisting}[style=pyin,caption={Tracing a frontier and reading the
maximum-Sharpe portfolio.},label={lst:basic}]
import numpy as np
...
solving through the Woodbury identity instead of forming $\Sig$
(Listing~\ref{lst:factor}). A short-sample sample covariance is handled the same
way through \texttt{GramCovariance}, built straight from the centred return matrix.

\begin{lstlisting}[style=pyin,caption={The factor backend traces the identical
frontier without ever forming the $n\times n$ matrix.},label={lst:factor}]
...
General linear constraints are passed the same way, exactly the program
\eqref{eq:qp}: a weighted or multi-row $A,b$ for sector or factor neutrality, and
$G,h$ for group or sector exposure caps (a $\ge$ floor is the negated row). The
builder accumulates rows, so a neutrality block sits alongside the budget while the
box bounds remain the cheaper per-variable active set of \S\ref{sec:events}.
Listing~\ref{lst:equality} adds a sector-neutrality row to the budget (a two-row
$A$), so the first turning point is the linear program of \S\ref{sec:first} rather
than the greedy fill, and the traced frontier holds the sector exposure at its
target throughout.

\begin{lstlisting}[style=pyin,caption={A multi-row equality $A,b$: budget plus a
sector-neutrality row. Reuses \texttt{mean}, \texttt{Sigma} from
Listing~\ref{lst:basic}.},label={lst:equality}]
...
A coupling inequality is added the same way. Listing~\ref{lst:inequality} caps each
sector's exposure with a block $G w \le h$; the high-return end would overweight a
sector, so the cap is \emph{active} at every turning point of this frontier.

\begin{lstlisting}[style=pyin,caption={An inequality block $G w \le h$: a
per-sector exposure cap, active along the whole trace.},label={lst:inequality}]
...
Both trace the exact constrained frontier, not an approximation:
\S\ref{sec:exactness} validates each against a per-$\lambda$ general QP solve,
including the regime where the inequality caps bind. Every figure
and table in the remainder of this paper is regenerated by a single replication
script, \srcfile{experiments/reproduce_paper.py}{reproduce\_paper.py},
...
familiar open CLA, not because it is a tuned competitor; the gap below therefore
measures the cost of how its per-step work is organised (\S\ref{sec:baseline}), not
\texttt{cvxcla} against a performance-optimised solver. The number of turning points tracks $n$ closely
($21, 42, 81, 159, 322, 641$ respectively, and PyPortfolioOpt finds nearly the same
set: $19, 42, 81, 158, 322, 641$), confirming the $O(n)$ count of
\S\ref{sec:experiment}. The small count differences are not noise but a completeness
gap: PyPortfolioOpt's turning points are a strict \emph{subset} of \texttt{cvxcla}'s
(every PyPortfolioOpt corner is matched), and \texttt{cvxcla} finds one or two
\emph{additional} genuine corners that PyPortfolioOpt omits (the omitted weights
differ from PyPortfolioOpt's nearest reported corner by up to $\sim\!2\times10^{-2}$,
well above round-off). Since \texttt{cvxcla}'s frontier is independently validated
exact (\S\ref{sec:exactness}), these are corners PyPortfolioOpt misses, a (minor)
correctness difference, not merely a speed one. The two
...
of $\approx 2.1$ for the \texttt{cvxcla} dense backend and $\approx 1.3$ for the
...
away as $n$ grows: from rough parity at $n=40$ to a $\approx 6.0\times$ speedup
at $n=640$ (Table~\ref{tab:scaling}). This $\approx 6.0\times$ is the genuine
...
($\approx n^{3.4}$): at $n=640$ it takes about $255$~seconds, so \texttt{cvxcla}'s
dense backend is roughly $397\times$ faster and the factor backend roughly
$2370\times$ faster. This gap is not a \texttt{numpy}-versus-Python artefact (
...
\midrule
$20$  & $21$  & $7.1$        & $0.7$   & $1.4$   & $1.5$ \\
$40$  & $42$  & $39.9$       & $1.9$   & $3.5$   & $3.4$ \\
$80$  & $81$  & $207.9$      & $4.6$   & $7.7$   & $6.9$ \\
$160$ & $159$ & $1{,}470$    & $12.6$  & $23.1$  & $14.6$ \\
$320$ & $322$ & $16{,}632$   & $49.2$  & $100.1$ & $35.1$ \\
$640$ & $641$ & $254{,}726$  & $307.6$ & $642.0$ & $107.3$ \\
\bottomrule
\end{tabular}
...
$\approx n^{2.1}$ (\texttt{cvxcla} dense) and $\approx n^{1.3}$
(\texttt{cvxcla} factor).}
\label{tab:scaling}
\end{table}
...
$\approx n^{2.1}$; and its diagonal-plus-low-rank factor backend, solving the
identical covariance through the Woodbury identity, like $\approx n^{1.3}$. At
$n = 640$ the factor backend is roughly $2370\times$ faster than PyPortfolioOpt and
$6.0\times$ faster than the dense backend. The incremental-inverse baseline and the
dense backend lie close together, far below PyPortfolioOpt; that gap reflects how
PyPortfolioOpt organises its per-step work, not the underlying critical-line
algorithm (\S\ref{sec:baseline}). The orange
curve reconstructs the frontier with Clarabel \citep{clarabel} as $n$ QP solves over
a $\lambda$-grid; it is drawn here only for context and is discussed in
\S\ref{sec:clarabel}, not in this section. Markers are the median of $3$
repetitions; the shaded band around each spans their min and max.}
\label{fig:scaling}
\end{figure}
...
The $\approx 397\times$ gap between \texttt{cvxcla}'s dense backend and
...
inverses per turning point (in the reference code of \citet{bailey2013} the call
\texttt{np.linalg.inv(covarF)} sits inside the per-candidate \texttt{for i in b}
loop that searches for the asset to free; PyPortfolioOpt inherits this structure). \texttt{cvxcla} instead forms the affine segment once
...
The answer is yes: the incremental-inverse baseline ($\approx n^{2.0}$, $308$~ms at
$n=640$) is about $2.1\times$ \emph{faster} than the dense backend
($\approx n^{2.1}$, $642$~ms), exactly as the $O(n_\F^2)$-versus-$O(n_\F^3)$ count
...
backend more than repays it: at $n=640$ it is $\approx 6.0\times$ faster than the
dense backend and $\approx 2.9\times$ faster than the incremental-inverse baseline,
...
and Table~\ref{tab:rank} show the crossover: the factor backend is $2.8\times$
...
\midrule
$20$  & $102.1$ & $36.1$  & $2.8\times$ \\
$40$  & $100.2$ & $45.2$  & $2.2\times$ \\
$80$  & $99.0$  & $58.9$  & $1.7\times$ \\
$160$ & $83.9$  & $87.8$  & $1.0\times$ \\
$320$ & $81.0$  & $199.9$ & $0.4\times$ \\
\bottomrule
\end{tabular}
...
The midpoint is a representative interior point of each segment; because the segment
is affine in $\lambda$, agreement at one interior point certifies the whole open
segment, and its endpoints are the turning points, which are themselves validated to
satisfy the bounds and the constraints (\S\ref{sec:events}) as they are recorded.

The same check extends to general linear constraints, the regime the headline
examples do not exercise. On a deterministic $30$-asset factor market we trace two
constrained frontiers and re-solve every segment midpoint as a general QP carrying
the same constraints
(\srcfile{experiments/validate_constraints.py}{validate\_constraints.py}):
first a budget together with a characteristic-neutrality row (a two-row equality
system, $34$ turning points, so the maximum-return vertex is the linear program of
\S\ref{sec:first} rather than the greedy fill), and second a budget with per-sector
exposure caps $G w \le h$ ($31$ turning points, with an inequality row \emph{active}
at $30$ of them, so the bordered solve of \S\ref{sec:lars} is genuinely exercised).
In both the affine segments match the QP optimiser to solver precision (maximum
weight discrepancy $\approx 6\times 10^{-10}$), confirming that the inequality and
multi-row-equality machinery traces the exact constrained frontier, not merely a
...
points}) in a median of $\approx 18$~ms (Figure~\ref{fig:realfrontier}); a
rank-$20$ diagonal-plus-low-rank approximation of the same covariance traces a
near-identical frontier ($89$ turning points) in $\approx 10$~ms through the
...
\paragraph{The combinatorial axis: a tie-heavy stress test.}
The guarantee above is for rank and conditioning; the orthogonal, combinatorial axis
is event ties, handled by the Bland-style rule of \S\ref{sec:bland}. We characterise
it empirically rather than leave it asserted
(\srcfile{experiments/tie_degeneracy.py}{tie\_degeneracy.py}): across four
deliberately degenerate families and $128$ instances ($n$ up to $240$), exactly-tied
means and exact asset duplicates both trace to completion ($64/64$) -- the duplicates
included because the tie-break never frees both copies of a pair, so the singular
covariance never reaches the free block; a constraint-heavy family of $p\approx n/3$
group caps also completes ($32/32$) with as many as $53$ inequality rows active at a
single turning point; and an over-constrained family of $p=n$ overlapping caps is
\emph{declined} at the maximum-return vertex ($32/32$), the documented
degenerate-vertex boundary, rather than mis-solved. No instance reached the iteration
cap, and every completed trace used at most $1.3\%$ of it. This is empirical
reassurance on the combinatorial axis, not the worst-case proof the rank/conditioning
guard provides, and constructing an adversarial cycling instance remains open.

\subsection{A general-solver baseline: a QP swept over \texorpdfstring{$\lambda$}{lambda}}
\label{sec:clarabel}

The comparisons so far are against other critical-line implementations. The
complementary question is how the exact trace compares to the route a general
convex-optimisation user would actually take: hand the quadratic program
\eqref{eq:qp} at a fixed $\lambda$ to a solver and sweep $\lambda$ to recover the
frontier (\S\ref{sec:bailey}). We make this concrete with Clarabel
\citep{clarabel}, the interior-point conic solver that CVXPY selects by default,
driven through its \emph{native} Python API with no modelling layer in between, so
the baseline carries no per-solve canonicalisation overhead and the comparison is
as favourable to it as possible. For each universe we solve \eqref{eq:qp} at $n$
values of $\lambda$ spanning the frontier (a grid as fine as the universe is large)
and time the whole sweep against a single \texttt{cvxcla} trace
(\srcfile{experiments/clarabel_baseline.py}{experiments/clarabel\_baseline.py}).

Even with the modelling layer stripped away, the grid is slower at every size and
the gap widens with $n$: from $\approx 1.7\times$ at $n=20$ to $\approx 69\times$ at
$n=320$, where one \texttt{cvxcla} trace takes $\approx 0.10$~s and the $320$
Clarabel solves take $\approx 7.0$~s ($\approx 22$~ms each). The sweep grows like
$\approx n^{2.9}$ -- $n$ solves, each an interior-point QP whose own cost climbs
with $n$ -- far steeper than the trace (Figure~\ref{fig:clarabel}). And the grid
only \emph{samples} the frontier: each Clarabel solution matches the exact
piecewise-linear frontier (read off \texttt{cvxcla}'s turning points by
interpolation in $\lambda$, an independent exactness check alongside
\S\ref{sec:exactness}) to the solver's tolerance, a maximum weight discrepancy of
$\approx 4\times 10^{-3}$ at Clarabel's default settings, whereas the trace returns
the frontier exactly and in full. The reading is the one of \S\ref{sec:bailey}: a
general solver is the right tool for a \emph{single} risk target, but reconstructing
the whole frontier by gridding is both slower and only approximate.

\begin{figure}[h]
\centering
\includegraphics[width=0.72\textwidth]{clarabel_baseline.pdf}
\caption{Time to obtain the frontier as a function of the number of assets $n$
(log--log): a single exact \texttt{cvxcla} trace against reconstructing the frontier
with $n$ Clarabel \citep{clarabel} QP solves over a $\lambda$-grid (the solver
driven natively, no CVXPY layer). The grid sweep grows like $\approx n^{2.9}$ and is
$\approx 69\times$ slower than the trace at $n=320$, and yields only an $n$-point
sampling rather than the exact piecewise-linear frontier.}
\label{fig:clarabel}
\end{figure}

\section{A reusable path tracer: general constraints and the \texttt{Lasso} client}
\label{sec:lars}

The turning-point loop never refers to portfolios except through the data
$(\Sig, \mu)$ and the feasible set. Two consequences of this are worth recording, both
purely about the implementation; the broader correspondence they reflect, with least
angle regression and the LASSO homotopy of statistical learning, and its history, are
the subject of a companion paper \citep{schmelzer2026perspective} and are not revisited
here.

\paragraph{General linear inequalities.} Beyond box bounds and the equality system
$A w = b$, \texttt{cvxcla} admits arbitrary linear inequalities $G w \le h$ from
\eqref{eq:qp} at no structural cost. The mechanism is the row analogue of the box active
set: a box bound is a per-variable inequality (an asset is free or blocked), while a row
of $G w \le h$ is a per-row inequality (active at $g_i\trans w = h_i$, or slack), and the
events are the same ``smallest positive step to the next breakpoint'', scanned over the
$p$ rows of $G$ in addition to the $n$ box coordinates (\S\ref{sec:events}). An active
inequality row enters the reduced KKT system \eqref{eq:kkt} exactly as an equality row:
stacking $C = [\,A;\,G_{\Sset}\,]$ over the active rows recovers the equality-only solve
with $A$ replaced by $C$, so each step is still a solve against the free covariance block
$\Sig_{\F\F}$ (through the operator interface of \S\ref{sec:operators}, never an
$n\times n$ matrix) feeding an $(m+|\Sset|)\times(m+|\Sset|)$ Schur complement. The box
bounds remain a separate, cheaper per-variable active set rather than being folded into
$G$, so the common box-only problem pays nothing for the generalisation. Turning an
inequality into an equality with a non-negative slack does \emph{not} work: the slack
carries no curvature, so the augmented Hessian $\operatorname{diag}(\Sig, 0)$ is only
positive semidefinite and the reduced solve is singular whenever a slack is free; the
bordered system above and a per-row event are used instead.

\paragraph{A second client of the tracer.} Because the loop reaches the quadratic form
only through the operator interface, it factors into a problem-agnostic path tracer
(\texttt{cvxcla.pathtracer.trace}) of which the \texttt{CLA} is one client. The package
ships a second, \texttt{Lasso} (\srcfile{src/cvxcla/lasso.py}{src/cvxcla/lasso.py}),
driven by the \emph{same} \texttt{trace} loop and the same Bland event selection; only
the segment solve and the meaning of an event differ. It supplies the Gram matrix
$X\trans X$ and the linear term $X\trans y$ and traces the entire LASSO regularisation
path from $\lambda_{\max} = \lVert X\trans y\rVert_\infty$ (where $\beta = 0$) down to
the least-squares fit at $\lambda = 0$; the same $G\beta \le h$ rows and a
non-negativity restriction are accepted exactly as for the \texttt{CLA}. As a regression
test that the shared engine is correct, the traced path matches scikit-learn's
\texttt{lars\_path} \citep{scikitlearn} breakpoint by breakpoint on a standardised
$60\times 12$ design (to $\approx 3\times 10^{-15}$ over all $13$ breakpoints,
Listing~\ref{lst:lasso}), and the constrained and non-negative paths match a
per-$\lambda$ QP to $\approx 2\times10^{-10}$
(\srcfile{experiments/validate_lasso_constraints.py}{validate\_lasso\_constraints.py}).
We carry this only as evidence that the tracer is genuinely problem-agnostic, the
software counterpart of the operator abstraction of \S\ref{sec:operators}.

\begin{lstlisting}[style=pyin,caption={The CLA path tracer drives a LASSO through
\texttt{cvxcla.Lasso}, matched against scikit-learn's \texttt{lars\_path}.},label={lst:lasso}]
import numpy as np
...
\bibitem[Nicolini et~al.(2025)]{skfolio}
C.~Nicolini, M.~Manzi, and H.~Delatte.
\newblock skfolio: Portfolio optimization in {Python}.
\newblock arXiv:2507.04176, 2025.
...
\bibitem[Schmelzer(2026)]{schmelzer2026perspective}
T.~Schmelzer.
\newblock Parametric active-set homotopies: a common computational structure across
statistics, optimization, and finance.
\newblock Companion working paper, 2026.
