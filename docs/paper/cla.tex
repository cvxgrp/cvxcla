...
homotopy; the two solution paths coincide as point sets under a gross-exposure
constraint, and one path-tracing engine, carrying the same linear constraints,
returns both the exact constrained efficient frontier and the exact constrained LASSO
path (a parameter-level bijection holds under an additional regularity condition
detailed in~\S\ref{sec:lars}).
...
(\S\ref{sec:lars}).  The two solution paths coincide as point sets under a
gross-exposure constraint; a bijection on parameter values holds under an additional
regularity condition (Theorem~\ref{thm:cla-lasso} and
Proposition~\ref{prop:regularity}).  The \texttt{GramCovariance} backend makes
the correspondence literal.
...
\begin{theorem}[CLA--LASSO path coincidence]\label{thm:cla-lasso}
Under a gross-exposure constraint $\|w\|_1 \le c$ with short positions admitted
($\lb_i < 0$ for some $i$), the CLA solution path $\lambda \mapsto w(\lambda)$ and
the LASSO regularisation path $c \mapsto \beta(c)$ are two homotopies over the same
parametric quadratic programme.
\begin{enumerate}
\item[(a)] \textbf{Point-set coincidence.} The two paths coincide as solution curves:
every CLA turning point $w(\lambda_k)$ is a LASSO breakpoint, and every LASSO
breakpoint is a CLA turning point.
\item[(b)] \textbf{Bijection on parameters (conditional).} The gross-exposure
reparametrisation $c(\lambda) = \|w(\lambda)\|_1$ maps the CLA turning-point
parameter values $\{\lambda_k\}$ bijectively onto the LASSO breakpoint values
$\{c_k\}$ if and only if $c'(\lambda) \neq 0$ on every interior segment.  This
regularity condition is characterised in Proposition~\ref{prop:regularity}.
\end{enumerate}
\end{theorem}

\begin{proposition}[Bijection regularity]\label{prop:regularity}
On a CLA segment with affine direction $\beta$ (so $w(\lambda) = \alpha + \lambda\beta$)
and free-set sign vector $s_\F = \operatorname{sign}(w_\F(\lambda))$, the
gross-exposure derivative is
\[
c'(\lambda) \;=\; s_\F\trans\beta_\F.
\]
The map $\lambda \mapsto c(\lambda)$ is locally injective on that segment if and only
if $s_\F\trans\beta_\F \neq 0$.  The condition fails---and the bijection of
Theorem~\ref{thm:cla-lasso}(b) degenerates---when the active constraints force the
free-asset direction to have zero net signed mass ($s_\F\trans\beta_\F = 0$), so that
the gross-exposure level is constant over a positive $\lambda$-interval and multiple
risk-aversion values correspond to the same $c$.
\end{proposition}

\subsection{A LASSO path through the same engine}
\label{sec:lassodemo}

The correspondence is not only conceptual: the package makes it executable. The
class \texttt{Lasso} (\srcfile{src/cvxcla/lasso.py}{src/cvxcla/lasso.py}) is driven by the
\emph{same} \texttt{cvxcla.pathtracer.trace} loop and the same Bland event
selection as the \texttt{CLA}; only the segment solve and the meaning of an event
differ. It supplies the Gram matrix $X\trans X$ (as a \texttt{DenseCovariance}, the
backend named \texttt{QuadraticForm} precisely because it is not specific to a
covariance) and the linear data $X\trans y$, and traces the entire LASSO
regularisation path from $\lambda_{\max}=\lVert X\trans y\rVert_\infty$ (where
$\beta=0$) down to the least-squares fit at $\lambda=0$. As a check that this is the
\emph{same} homotopy the reference statistical-learning solvers trace, we compare it
breakpoint by breakpoint against scikit-learn's \texttt{lars\_path}
\citep{scikitlearn} on a standardised $60\times 12$ regression
(Listing~\ref{lst:lasso}); the two paths coincide to solver precision (maximum
coefficient discrepancy $\approx 3\times 10^{-15}$ over all $13$ breakpoints,
Figure~\ref{fig:lasso}). The experiment is reproducible from
\srcfile{experiments/lasso_path.py}{experiments/lasso\_path.py}.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.72\textwidth]{lasso_path.pdf}
\caption{The LASSO regularisation path of a standardised $60\times 12$ regression,
traced by \texttt{cvxcla.Lasso} through the same parametric active-set engine as the
efficient frontier. Each line is one coefficient $\beta_j(\lambda)$ as the penalty
$\lambda$ decreases from $\lambda_{\max}$ (right) to $0$ (left); the markers are the
breakpoints reported independently by scikit-learn's \texttt{lars\_path}. The two
implementations agree to solver precision ($\approx 3\times 10^{-15}$), confirming
