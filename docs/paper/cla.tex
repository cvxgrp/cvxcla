...
points}) in a median of $\approx 18$~ms (Figure~\ref{fig:realfrontier}). The
annualised frontier spans expected returns of
...
\paragraph{Real-data payoff of the structured backend.}
The structured backend's advantage is not a claim about simulated inputs: it
shows on this real S\&P 500 problem as well. We replace the full $494\times494$
sample covariance with a rank-$K$ diagonal-plus-low-rank approximation
$\widehat\Sig_K = \operatorname{diag}(d) + U_K\,\Delta_K\,U_K\trans$ (the top-$K$
eigenpairs plus the diagonal residual, standard practice for a structured risk
model) and trace the frontier through the Woodbury backend at $K \in
\{5,10,20,40\}$. Table~\ref{tab:realtiming} and Figure~\ref{fig:realtiming} report
the results. For low ranks ($K=5$--$10$) the factor backend is measurably faster
than the dense solve of the full covariance; at $K=20$ (the rank more typical of
production risk models) it remains faster; and the advantage shrinks toward parity
as $K$ grows toward $N$. The pattern exactly mirrors the simulated rank sweep
of \S\ref{sec:rankscaling}: the structured backend wins precisely in the regime
real risk models inhabit, $K \ll N$.

\begin{table}[h]
\centering
\begin{tabular}{rrrr}
\toprule
$K$ & Turning points & Factor [ms] & Speedup vs dense \\
\midrule
\multicolumn{2}{l}{Dense ($N=494$)} & $\approx 18$ & --- \\
$5$  & $\approx 88$ & $\approx 7$  & $\approx 2.5\times$ \\
$10$ & $\approx 81$ & $\approx 7$  & $\approx 2.5\times$ \\
$20$ & $\approx 89$ & $\approx 10$ & $\approx 1.8\times$ \\
$40$ & $\approx 90$ & $\approx 12$ & $\approx 1.5\times$ \\
\bottomrule
\end{tabular}
\caption{Frontier trace time on the S\&P 500 dataset ($N=494$ assets,
$T=1213$ days) with the dense sample covariance and with rank-$K$
diagonal-plus-low-rank approximations at four values of $K$
(median of $5$ repetitions; indicative values, machine-dependent --- see
the benchmark environment note at the start of \S\ref{sec:experiment}).
All four factor frontiers are near-identical ($\pm1$ corner) to the dense
frontier. The structured backend wins robustly for $K\ll N$, matching the
simulated scaling of Table~\ref{tab:rank}.}
\label{tab:realtiming}
\end{table}

\begin{figure}[h]
\centering
\includegraphics[width=0.72\textwidth]{real_timing.pdf}
\caption{Dense vs factor trace time on the real S\&P 500 dataset ($N=494$
assets) at factor ranks $K\in\{5,10,20,40\}$. The dense bar (red) is the same
height in every pair; the factor bars (blue) shrink with decreasing $K$. The
speedup annotations confirm the structured backend's advantage holds on real
data, not only on simulated problems, and diminishes as $K$ grows toward $N$
(consistent with Figure~\ref{fig:rank}).}
\label{fig:realtiming}
\end{figure}
