...
against degeneracy and floating-point noise. This hardening is bounded, not total:
near-degenerate problems complete by projecting sub-tolerance round-off back onto
the feasible set, but a numerically rank-deficient free block, and a degenerate
maximum-return vertex under a general equality system, are declined with a
diagnosis rather than mis-solved, and the lowest-index tie-break, robust on the
degenerate cases we test, is not proven to cover every adversarial tie-heavy
instance.
\end{abstract}
...
endpoints, taken over the two segments bracketing the discrete Sharpe maximum. The
...
General linear constraints are passed the same way: a weighted or multi-row $A,b$
for sector or factor neutrality, and $G,h$ for group or sector exposure caps (a
$\ge$ floor is the negated row), exactly the program \eqref{eq:qp}. Every figure
and table in the remainder of this paper is regenerated by a single replication
script, \srcfile{experiments/reproduce_paper.py}{experiments/reproduce\_paper.py},
...
($\approx n^{3.4}$): at $n=640$ it takes about $249$~seconds, so \texttt{cvxcla}'s
dense backend is roughly $312\times$ faster and the factor backend roughly
$2390\times$ faster. This gap is not a \texttt{numpy}-versus-Python artefact (
...
\midrule
$20$  & $21$  & $7.2$        & $0.7$   & $1.3$   & $1.5$ \\
$40$  & $42$  & $40.7$       & $1.9$   & $3.5$   & $3.4$ \\
$80$  & $81$  & $210.5$      & $4.7$   & $7.8$   & $6.8$ \\
$160$ & $159$ & $1{,}458$    & $12.6$  & $26.1$  & $15.1$ \\
$320$ & $322$ & $16{,}635$   & $52.2$  & $116.5$ & $36.4$ \\
$640$ & $641$ & $249{,}106$  & $324.3$ & $799.6$ & $104.4$ \\
\bottomrule
\end{tabular}
...
$n = 640$ the factor backend is roughly $2390\times$ faster than PyPortfolioOpt and
$7.7\times$ faster than the dense backend. The incremental-inverse baseline and the
dense backend lie close together, far below PyPortfolioOpt; that PyPortfolioOpt gap
reflects how this teaching-oriented reference implementation organises its per-step
work, not the underlying critical-line algorithm (\S\ref{sec:baseline}). The orange
...
The $\approx 312\times$ gap between \texttt{cvxcla}'s dense backend and
...
The answer is yes: the incremental-inverse baseline ($\approx n^{2.0}$, $324$~ms at
$n=640$) is about $2.5\times$ \emph{faster} than the dense backend
($\approx n^{2.2}$, $800$~ms), exactly as the $O(n_\F^2)$-versus-$O(n_\F^3)$ count
...
dense backend and $\approx 3.1\times$ faster than the incremental-inverse baseline,
