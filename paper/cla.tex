...
frontier of a $200$-asset problem estimated from $2000$ daily return
observations. The experiment is reproducible: it is implemented in
\texttt{experiments/frontier\_200x2000.py} and uses a fixed random seed.

\paragraph{Setup.}
We simulate $T = 2000$ daily returns for $N = 200$ assets from a $K = 15$ factor
model --- each asset loads on $15$ common factors plus a well-conditioned
idiosyncratic shock, with a dispersed cross-section of expected returns. From the
simulated panel we form the sample mean vector $\mu \in \R^{200}$ and the sample
covariance matrix $\Sig \in \R^{200\times 200}$; because $T > N$ the sample
covariance is full rank (its condition number here is $\approx 1.5\times 10^{3}$).
We then trace the long-only, fully-invested frontier, i.e.\ \eqref{eq:qp} with the
...
\textbf{202 turning points} (Figure~\ref{fig:frontier}); between consecutive
points the frontier is the exact affine segment \eqref{eq:segment}, so these
$202$ corner portfolios describe the whole efficient set with no discretisation.
Tracing the full frontier with the dense backend takes a median of
$\approx 38$~ms (about $0.19$~ms per turning point) on a single core of a
commodity laptop. Replacing the dense covariance with a diagonal-plus-low-rank
factor estimate of $\Sig$ (its leading $15$ eigenpairs plus a diagonal residual,
solved through the Woodbury identity of \S\ref{sec:operators}) traces the same
frontier --- $201$ turning points, visually indistinguishable --- in
$\approx 19$~ms, a $2.0\times$ speedup, confirming that the covariance-operator
abstraction changes only the cost of each solve and not the frontier it produces.
Table~\ref{tab:experiment} collects the figures.
...
\midrule
Turning points        & $202$            & $201$ \\
Median trace time      & $\approx 38$~ms  & $\approx 19$~ms \\
Time per turning point & $\approx 0.19$~ms & $\approx 0.09$~ms \\
\bottomrule
\end{tabular}
\caption{Tracing the full $200$-asset frontier (median of $5$ repetitions).
The factor backend never materialises the $200\times 200$ matrix and is about
twice as fast while recovering the same frontier.}
...
Algorithm. Each marker is one of the $202$ turning points; the endpoints are the
...
The trace runs cleanly because the expected returns are well separated and the
