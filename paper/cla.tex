...
It traces the same turning points as the method described here --- the
underlying mathematics is Markowitz's --- but differs in four ways that matter in
practice.
...
\item \textbf{Linear algebra.} Where the reference algorithm maintains an explicit
inverse of the free block, \texttt{cvxcla} solves the reduced KKT system by block
elimination: a single multi-right-hand-side solve against $\Sig_{\F\F}$ feeds an
$m \times m$ Schur complement \eqref{eq:schur} that handles the equality
constraints. The $\alpha$ and $\beta$ systems reuse the same factorisation, and
the covariance enters only as a black-box solve --- which is what makes the
backend abstraction possible.
...
A further, non-algorithmic difference is implementation: \texttt{cvxcla}
vectorises the per-step linear algebra over \texttt{numpy}, whereas the reference
implementation is written as explicit Python loops. As \S\ref{sec:experiment}
...
$264$~seconds, so \texttt{cvxcla}'s dense backend is roughly $340\times$ faster
and the factor backend roughly $2900\times$ faster. This gap must be read with
care --- it conflates two effects, which \S\ref{sec:baseline} disentangles with a
vectorised explicit-inverse baseline. The short version: the dense-over-PyPortfolioOpt
factor is \emph{entirely} implementation (vectorised \texttt{numpy} versus
element-wise Python loops); the only genuine \emph{algorithmic} advantage over a
well-implemented CLA is the structured factor backend.
...
$O(n)$-point frontier. ``Inverse baseline'' is the vectorised
explicit-inverse CLA of \S\ref{sec:baseline}
(\texttt{experiments/inverse\_cla.py}): PyPortfolioOpt's incremental-inverse
linear algebra, vectorised over \texttt{numpy} like \texttt{cvxcla}. Empirically
the times scale like $\approx n^{3.4}$ (PyPortfolioOpt), $\approx n^{2.0}$
(inverse baseline), $\approx n^{2.2}$ (\texttt{cvxcla} dense) and
$\approx n^{1.3}$ (\texttt{cvxcla} factor).}
\label{tab:scaling}
\end{table}
...
$n = 640$ the factor backend is roughly $2900\times$ faster than PyPortfolioOpt and
$8\times$ faster than the dense backend. The baseline and the dense backend nearly
coincide, far below PyPortfolioOpt: the gap to PyPortfolioOpt is vectorisation, not
algorithm.}
\label{fig:scaling}
\end{figure}

\subsection{Isolating vectorisation from the algorithm}
\label{sec:baseline}

The $\approx 340\times$ gap between \texttt{cvxcla}'s dense backend and
PyPortfolioOpt at $n=640$ has two possible sources, and it matters which one
dominates. One is \emph{implementation}: \texttt{cvxcla} vectorises the per-step
linear algebra over \texttt{numpy}, while PyPortfolioOpt is written as element-wise
Python loops. The other is \emph{strategy}: where PyPortfolioOpt à la
\citet{bailey2013} maintains an explicit inverse of the free block and updates it
incrementally as one asset enters or leaves, \texttt{cvxcla} discards that inverse
and re-solves the block-eliminated KKT system from scratch each step
(\S\ref{sec:blockelim}). To separate the two we add a third implementation
(\texttt{experiments/inverse\_cla.py}): a CLA that keeps PyPortfolioOpt's
incremental-inverse strategy --- maintaining $\Sig_{\F\F}\inv$ through rank-one
bordered (asset enters) and deletion (asset leaves) updates --- but vectorises it
over \texttt{numpy} exactly as \texttt{cvxcla} does. It traces the identical
turning points to machine precision (max.\ weight discrepancy $<10^{-13}$ across
$n=20$--$160$), so a timing comparison is clean.

The decomposition at $n=640$ is unambiguous (Table~\ref{tab:scaling}). The
vectorised baseline runs in $326$~ms versus PyPortfolioOpt's $264$~seconds --- a
$\approx 810\times$ speed-up from \emph{vectorisation alone}, the two
implementations running the same algorithm. The remaining step, from the baseline's
incremental inverse to \texttt{cvxcla}'s fresh block-eliminated solve, is not a
speed-up at all: the baseline ($\approx n^{2.0}$) is in fact about $2.4\times$
\emph{faster} than the dense backend ($\approx n^{2.2}$, $775$~ms), exactly as one
would expect since a rank-one inverse update costs $O(n_\F^2)$ against the fresh
solve's $O(n_\F^3)$. In other words, \texttt{cvxcla}'s advantage over PyPortfolioOpt
on a dense covariance is \emph{entirely} vectorisation; the block-elimination
strategy is, on its own, marginally slower than a well-maintained explicit inverse.

This is not a defect but a deliberate trade. \texttt{cvxcla} re-solves rather than
updating an inverse because the fresh solve is what composes with the covariance
operator (\S\ref{sec:operators}): the factor backend never forms $\Sig_{\F\F}$ at
all, so there is no inverse to update --- it solves through the Woodbury identity.
The dense backend gives up the modest incremental-inverse win to keep one
turning-point loop that runs over \emph{any} backend, and the factor backend then
more than repays it: at $n=640$ it is $\approx 8.4\times$ faster than the dense
backend and $\approx 3.5\times$ faster than the incremental-inverse baseline, the
only one of the four implementations with a genuinely sub-quadratic exponent
...
and Table~\ref{tab:rank} show the crossover: the factor backend is $3.2\times$
...
\midrule
$20$  & $109.1$ & $33.8$  & $3.2\times$ \\
$40$  & $110.9$ & $42.6$  & $2.6\times$ \\
$80$  & $106.2$ & $58.0$  & $1.8\times$ \\
$160$ & $86.5$  & $83.2$  & $1.0\times$ \\
$320$ & $86.1$  & $194.3$ & $0.4\times$ \\
\bottomrule
\end{tabular}
