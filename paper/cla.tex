...
dense strategy. It is used anyway because the fresh solve is what composes with the
covariance operator (\S\ref{sec:operators}): the factor backend never forms
$\Sig_{\F\F}$ at all, so there is no inverse to maintain --- it solves through the
Woodbury identity. The dense backend gives up the modest incremental-inverse win to
